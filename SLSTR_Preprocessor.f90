!--------------------------------------------------------------------------------------------
!M+ SLSTR_Preprocessor
! NAME:
!     SLSTR_Preprocessor
!
! PURPOSE:
!>   Regrid VIS and SWIR observations at 1km using nearest neighbourhood and including orphan pixels
!
! CATEGORY:
!     Data I/O
!
...
MODULE SLSTR_Preprocessor
  ...
  !> Real number to represent missing values in I/O.  Must be set before calling this module.
  REAL :: MISSING_R

  !> Integer value to represent missing values in I/O.  Must be set before calling this module.
  INTEGER :: MISSING_I

  !> Whether to perform extra computation to process cosmetically filled IR pixels the same as the original pixels
  LOGICAL, PARAMETER :: align_cosmetic_pixels = .false.
  ...
  INTEGER, PARAMETER :: search_width = 8, search_height = 6     ! Search window size
  ...
  TYPE NEIGHBOURHOOD_ENTRY
    INTEGER, DIMENSION(k_nearest_neighbours) :: x
    INTEGER, DIMENSION(k_nearest_neighbours) :: y
    INTEGER, DIMENSION(k_nearest_neighbours) :: source
    REAL, DIMENSION(k_nearest_neighbours) :: squared_distances
...
CONTAINS
  ...
  !
  ! PURPOSE:
  !>    Passed a status code returned from a netcdf module operation, if the code indicates an error,
  !>    print more information and stop the program
  !
  ! CATEGORY:
  !
  ! LANGUAGE:
  ...
  !
  ! CALLING SEQUENCE:
  !     safe_get_real_data(ncid,1000,1000,'field1',arr,-99999)
  !
  ! ARGUMENTS:
  ...
  !
  ! CALLING SEQUENCE:
  !     safe_get_real_attribute(ncid,'field1','attr1')
  !
  ! ARGUMENTS:
  !>@ARG{ncid, in, INTEGER} ID of the opened file (returned from safe_open)
  ...
  FUNCTION safe_get_real_attribute(ncid,field_name,attr_name)
    ...
    ! -----------
    INTEGER, INTENT(IN) :: ncid
    CHARACTER (LEN = *), INTENT(IN) :: field_name, attr_name
  ...
  !
  ! CALLING SEQUENCE:
  !     safe_get_int_data(ncid,1000,1000,'field1',arr_of_int,-99999)
  !
  ! ARGUMENTS:
  !>@ARG{ncid, in, INTEGER} ID of the opened file (returned from safe_open)
  !>@ARG{name, in, CHARACTER(LEN=*)} name of the variable
  ...
  !
  ! PURPOSE:
  !>     insert a neighbour (squared-distance,source,x_index,y_index) into a neighbourhood that is already sorted
  ...
  !>@ARG{squared_distance, in, REAL} the source of the neighbour (orphan|main, a|b)
  ...
  !
  ! SIDE EFFECTS:
  !     the neighbourhood may be modified by adding a neighbour and potentially shuffling/removing existing ones
  !     the assigned may be increased by 1 if the number of neighbours stored in the neighbourhood increases
  !
  ! RESTRICTIONS:
  !
  ! PROCEDURE:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE insert_neighbour(neighbourhood,assigned,squared_distance,source,x_index,y_index)
    IMPLICIT NONE
    ...
    IF (assigned > 0) THEN
      ...
      DO insert_after_index=assigned,0,-1
        ...
        IF (insert_after_index < k_nearest_neighbours) THEN
    ...
    IF (insert_after_index < k_nearest_neighbours) THEN
      ...
      IF (assigned < k_nearest_neighbours) THEN
        assigned = assigned + 1
      END IF
    END IF
  ...
  !
  ! SIDE EFFECTS:
  !     Cells of (ir_x_index,ir_y_index,:) in neighbourhood arrays are assigned
  !
  ! RESTRICTIONS:
  !
  ! PROCEDURE:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE find_neighbours(ir_x_index,ir_y_index,ir_x,ir_y, &
    ...
    ! Local Variables
    ! ---------------
    REAL, ALLOCATABLE, DIMENSION(:,:) :: txmat, tymat, sqdist
    INTEGER :: status, w_width, w_height, row, col, loc1, i, j
    INTEGER :: vis_x_min_index, vis_x_max_index, vis_y_min_index, vis_y_max_index
    INTEGER :: vis_x_index, vis_y_index
    INTEGER, DIMENSION(1) :: loc
    REAL :: closest_sqdist, max_dist_sq
    INTEGER :: visible_width, visible_height
    INTEGER :: main_source, orphan_source

    INTEGER :: rows
    INTEGER :: cols
    INTEGER :: found, source, assigned
    REAL :: largest, v
    ...
    ! Calculate the width and height of the search window
    w_width = 1+vis_x_max_index-vis_x_min_index
    w_height = 1+vis_y_max_index-vis_y_min_index

    ! Allocate arrays to hold search window values from main and orphaned pixel data
    ALLOCATE(txmat(w_width+orphan_cartesian%valid_orphan_width,w_height),STAT=status)   ! across track radiance distance
    ALLOCATE(tymat(w_width+orphan_cartesian%valid_orphan_width,w_height),STAT=status)   ! along track radiance distance
    ALLOCATE(sqdist(w_width+orphan_cartesian%valid_orphan_width,w_height),STAT=status)  ! squared cartesian distance to IR pixel

    ! Assign the search window arrays with across and along track distances
    txmat(1:w_width,1:w_height) = &
        main_cartesian%xcoords(vis_x_min_index:vis_x_max_index,vis_y_min_index:vis_y_max_index)
    txmat(w_width+1:w_width+orphan_cartesian%valid_orphan_width,1:w_height) = &
        orphan_cartesian%xcoords(1:orphan_cartesian%valid_orphan_width,vis_y_min_index:vis_y_max_index)
    tymat(1:w_width,1:w_height) = &
        main_cartesian%ycoords(vis_x_min_index:vis_x_max_index,vis_y_min_index:vis_y_max_index)
    tymat(w_width+1:w_width+orphan_cartesian%valid_orphan_width,1:w_height) = &
        orphan_cartesian%ycoords(1:orphan_cartesian%valid_orphan_width,vis_y_min_index:vis_y_max_index)

    ! Calculate the squared cartesian distances
    sqdist = (txmat - ir_x)**2 + (tymat-ir_y)**2
    ! Propagate missing values from across track distance to the squared cartesian distances
    WHERE (txmat == MISSING_R)
      sqdist = 1.0e10
    END WHERE
    ...
    cols = SIZE(sqdist,1)
    rows = SIZE(sqdist,2)
    largest = 0  ! the value of the largest pixel currently in the neighbourhood
    found = 0  ! the number of pixels added to the neighbourhood
    assigned = 0

    ! iterate over the squared distance and maintain an unordered list of the closest K values
    ! in the min_values array, and the indexes/source of the values in the neighborhood
    DO row = 1,rows
      DO col = 1, cols
        v = sqdist(col,row)
        IF (v == MISSING_R .or. v >= max_dist_sq) THEN
          CYCLE ! ignore if the squared distance is null or too far away
        END IF
        vis_y_index = vis_y_min_index+row-1
        source = main_source
        IF (col <= w_width) THEN
          ! pixel is from the main pixels
          vis_x_index = vis_x_min_index+col-1
          ! check if this pixel is cosmetically filled, and ignore it if it is
          IF (IAND(main_cartesian%confidence(vis_x_index,vis_y_index),COSMETIC_PIXEL_MASK) /= 0) THEN
            CYCLE
          END IF
        ELSE
          ! pixel is from the orphan pixels
          vis_x_index = col-w_width
          source = orphan_source
        END IF

        CALL insert_neighbour(neighbourhood,assigned,v,source,vis_x_index,vis_y_index)
      END DO
    END DO
  ...
  !
  ! PURPOSE:
  !>     Find the K closest radiance band pixels to a every IR band pixel and store the resulting mapping
  !>     in a neighbourhood map.  Considers main and orphan band pixels.
  !
  ! CATEGORY:
  !
  ! LANGUAGE:
  ...
  !
  ! CALLING SEQUENCE:
  !     build_neighbourhood_map(ir_cartesian,main_cartesian,orphan_cartesian,neighborhood)
  !
  ! ARGUMENTS:
  ...
  !
  ! SIDE EFFECTS:
  !     Every value in neighbourhood is assigned
  !
  ! RESTRICTIONS:
  !
  ! PROCEDURE:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE build_neighbourhood_map(ir_cartesian,main_cartesian,orphan_cartesian,neighbourhood,stripe)
    IMPLICIT NONE
  ...
  !F-
  !------------------------------------------------------------------------------
  FUNCTION apply_function(function_code,values,value_count)
    IMPLICIT NONE
  ...
  !
  ! PURPOSE:
  !>    Aggregate radiance pixels to regrid onto the IR band.  Currently, this computes the mean value of the
  !>    neighbours.  Customise according to taste.
  !
  ! CATEGORY:
  !
  ! LANGUAGE:
  ...
  !
  ! CALLING SEQUENCE:
  !     CALL apply_neighbours(vis_radiance,vis_orphans,vis_exception,neighbourhood,vis_output_radiance)
  !
  ! ARGUMENTS:
  ...
  !
  ! SIDE EFFECTS:
  !     Every value in matrix vis_output_radiance is assigned
  !
  ! RESTRICTIONS:
  !
  ! PROCEDURE:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE apply_neighbours(vis_radiance_a,vis_orphans_a,vis_exception_a, &
    ...
    ! Local Variables
    ! ---------------
    INTEGER :: ir_x, ir_y, ik, n_x, n_y, n_s, exception
    REAL :: lookup
    INTEGER :: orphans_used, main_used
    INTEGER :: visible_width, visible_height, ir_width, ir_height, orphan_width, orphan_height
    INTEGER :: effective_k
    REAL, DIMENSION(k_nearest_neighbours) :: values
    INTEGER :: vcount
    ...
    IF (neighbourhood%include_a_stripe .and. neighbourhood%include_b_stripe) THEN
      ! when processing both a and b stripe, consider all neighbours
      effective_k = k_nearest_neighbours
    ELSE
      ! when processing only one stripe, consider the closest k/2 neighbours
      effective_k = k_nearest_neighbours / 2
    END IF
  ...
  ! PURPOSE:
  !>    Aggregate radiance pixels to regrid onto the IR band.  Currently, this computes the mean value of the
  !>    visible pixels at locations (2x,2y),(2x+1,2y),(2x+1,2y+1),(2x,2y+1)
  !
  ! CATEGORY:
  !
  ! LANGUAGE:
  ...
  !
  ! CALLING SEQUENCE:
  !     CALL apply_simple_aggregation(vis_radiance,vis_output_radiance)
  !
  ! ARGUMENTS:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE apply_simple_aggregation(vis_radiance,vis_output_radiance, function_code)
    IMPLICIT NONE
  ...
  !
  ! CALLS:
  !     safe_open, safe_get_real_data, safe_get_int_data, nf90_close, Path_Join
  !
  ! SIDE EFFECTS:
  !     Output arrays are allocated and all cells are assigned
  !
  ! RESTRICTIONS:
  !
  ! PROCEDURE:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE load_radiance_data(scene_folder,view_type,band,stripe,vis_radiance,vis_orphans,vis_exception)
    USE GbcsPath
    USE netcdf
    IMPLICIT NONE
  ...
  !
  ! CALLS:
  !     load_radiance_data, apply_neighbours (if neighbourhood allocated) else apply_simple_aggregation
  !
  ! SIDE EFFECTS:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE process_scene_band(view_type,scene_folder,band,vis_ir_radiance,neighbourhood,function_code)
    IMPLICIT NONE
    ...
    IF (ALLOCATED(neighbourhood%entries)) THEN
      ...
      CALL apply_neighbours(vis_radiance_a,vis_orphans_a,vis_exception_a, &
              vis_radiance_b,vis_orphans_b,vis_exception_b, &
              neighbourhood,vis_ir_radiance,function_code)
    ELSE
      CALL load_radiance_data(scene_folder,view_type,band,'a',vis_radiance_a,vis_orphans_a,vis_exception_a)
  ...
  !
  ! PURPOSE:
  !>     Try to adjust the cartesian coordinates of cosmetic pixels so that they exactly match the coordinates of the
  !>     pixels which contributed their values
  !
  ! CATEGORY:
  !
  ! LANGUAGE:
  ...
  !
  ! SIDE EFFECTS:
  !     Some cells in ir_cartesian.xcoords and ir_cartesian.ycoords are assigned
  !
  ! RESTRICTIONS:
  !
  ! PROCEDURE:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE align_cosmetics(ir_cartesian, s8_bt, ir_confidence)
    IMPLICIT NONE
    ...
    INTEGER :: cosmetic_count, miss_count, value_match_count
  ...
  !
  ! PURPOSE:
  !>    merge 2 neighbourhoods (presumably computed from different stripes) retaining the nearest K neighbours.
  !>    the second of the neighbourhoods will hold the merged neighbourhood
  !
  ! CATEGORY:
  !
  ! LANGUAGE:
  ...
  !
  ! SIDE EFFECTS:
  !     All cells in the neighborhood_out array are assigned
  !
  ! RESTRICTIONS:
  !
  ! PROCEDURE:
  ...
  !S-
  !------------------------------------------------------------------------------
  SUBROUTINE merge_neighbourhoods(neighbourhood_1_in,neighbourhood_2_inout)
    USE netcdf
    Use GbcsPath
    IMPLICIT NONE
    ...
    INTEGER :: width, height, x, y, nx, ny, i, assigned, source_a, source_b, a_idx, b_idx, source
    REAL :: value, value_a, value_b
    ...
    DO x = 1,width
      DO y = 1, height
        ...
        DO assigned = k_nearest_neighbours, 0, -1
          IF (assigned > 0) THEN
  ...
  !
  ! PURPOSE:
  !>     given a folder containing an SLSTR scene, a view (nadir or oblique) and stripe (a or b), loads the cartesian
  ...
  !>     bands from the same scene and view to a new IR based grid.  Neighbourhoods from a and b stripes
  !>     can also be merged by calling merge_neighbourhoods
  !
  ! CATEGORY:
  !
  ! LANGUAGE:
  ...
  !
  ! SIDE EFFECTS:
  !     All cells in the neighborhood array are assigned
  !
  ! RESTRICTIONS:
  !
  ! PROCEDURE:
